...
Multiple tools for different ASCII art needs. Local CLI tools are preferred; remote APIs require network access and must be refused when unavailable. No automatic installation or API keys are assumed.
...
- Requires network access; if network is unavailable, refuse the remote route and use a local fallback
- Works via curl when network access is available
...
# Remote URLs are network-dependent; refuse them when network is unavailable
```
...
9. **Any tool not installed** → do not install automatically; use the next local fallback, or refuse a remote route when network is unavailable
