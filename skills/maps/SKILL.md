...
Location intelligence using configured external map providers. 8 commands,
44 POI categories, zero dependencies (Python stdlib only), no API key
required by this client. Provider behavior and current data are not proven by
the local fixture audit.

Configured integrations are separated by function: geocoding and place
search use Nominatim/Overpass, routing uses OSRM, and timezone lookup uses
TimeAPI.io with a longitude-based approximation fallback after provider
errors. Do not treat a generic source label as proof of the source for every
field or command.
