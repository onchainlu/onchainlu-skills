...
Exit code 0 = clean, 1 = at least one layout problem; the JSON report lists
per-field `problems`. A requested overlay also follows the PDF skill's optional
render contract: unavailable rasterizers are reported without changing a clean
lint result, while an available backend failure exits 5. Lint the spec BEFORE
building — fixing numbers in JSON is cheaper than debugging a rendered PDF.
...
`"rendered": false` with dependency names). Feed the PNG to `vision_analyze`
